...
class Detect_class(object):
    ...
    def run(
                    ...
                    for c in det[:, -1].unique():
                        ...
                        if frame - tmp[names[int(c)]][1] < th:
                            ...
                            if tmp[names[int(c)]][1] - tmp[names[int(c)]][0] > th:
                                db[names[int(c)]].append(
                                    [
                                        round(tmp[names[int(c)]][0] / video_fps),
                                        round(tmp[names[int(c)]][1] / video_fps),
                                    ]
                                )
        ...
        for name in db:
            if tmp[name][1] - tmp[name][0] > th:
                db[name].append(
                    [round(tmp[name][0] / video_fps), round(tmp[name][1] / video_fps)]
                )
    ...
    def parse_opt(self):
        ...
        parser.add_argument(
            "--project", default="./output_video", help="save results to project/name"
        )
        parser.add_argument(
            "--name", default="output", help="save results to project/name"
        )
        parser.add_argument(
            "--exist-ok",
            action="store_true",
    ...
    def main(self, opt):
        print(
            colorstr("detect: ") + ", ".join(f"{k}={v}" for k, v in vars(opt).items())
        )
